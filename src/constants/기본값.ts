import type { Profile, User } from 'types-domain';

export const 리얼_월드_프로젝트_이름 = 'conduit';

export const 리얼_월드_도메인 = process.env.NEXT_PUBLIC_WEB_DOAMIN || 'http://localhost:3000';

export const 리얼_월드_서버_도메인 =
  process.env.NEXT_PUBLIC_SERVER_DOAMIN || 'https://api.realworld.io/api';

export const 리얼_월드_키 = 'rw-uid';

export const 리얼_월드_권한_필요_요청_키 = 'x-rw-auth-request';

export const 기본_프로필_이미지 = `https://static.productionready.io/images/smiley-cyrus.jpg`;

export const 페이지_컨텐츠_개수 = 20;

export const 기본_이미지_소스 = `data:image/png;base64,`;

export const 유저_스토리지_기본값: User = {
  email: '',
  token: '',
  username: '',
  bio: null,
  image: '',
};

export const 프로필_기본값: Profile = {
  username: '',
  bio: '',
  image: '',
  following: false,
};

export const 타임아웃_제한_시간 = 5000;

export const 하루 = 60 * 60 * 24;

export const 리액트_쿼리_기본값 = { defaultOptions: { queries: { retry: 1, staleTime: 하루 } } };
